...
pub fn skboot_authenticate(start: *const u32) -> Result<(), AuthenticateError> {
    // Note:
    ...
    let options = KbOptions {
        ...
        settings: KbSettings {
            authenticate: KbAuthenticate {
                profile: 0,
                min_build_number: 0,
                max_image_length: MAX_IMAGE_SIZE,
    ...
    if status != KbStatus::Success as u32 {
        defmt::error!("kinit failed {:?}", status);
        return Err(AuthenticateError::Fail);
    }

    let mut is_sign_verified: u32 = 1234;
    ...
    if cortex_m::peripheral::NVIC::is_enabled(mimxrt685s_pac::Interrupt::HASHCRYPT) {
        defmt::warn!("ROM API kept HASHCRYPT unmasked...");
    ...
    if status != KbStatus::Success as u32 {
        defmt::error!("kdeinit failed");
        return Err(AuthenticateError::Fail);
    }

    let status = BootStatus::try_from(result).map_err(|()| AuthenticateError::BootStatusUnknown)?;
    let is_sign_verified = SecureBool::try_from(is_sign_verified)
        .map_err(|()| AuthenticateError::IsSignVerifiedUnknown);
